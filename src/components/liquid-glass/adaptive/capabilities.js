...
export function parsePlatform() {
    ...
    const androidVersion = androidMatch ? Number.parseInt(androidMatch[1], 10) : null;
...
export function selectInitialQuality(gl, gpu, platform, benchmarkMs) {
    ...
    if (softwareRenderer || benchmarkMs > 95) {
        return { mode: 'glass', score: 0, reason: 'major-performance-caveat' };
    }

    let score = 0;
    score += cores >= 12 ? 4 : cores >= 8 ? 3 : cores >= 4 ? 2 : 0;
    score += memory >= 12 ? 3 : memory >= 8 ? 2 : memory >= 4 ? 1 : 0;
    score += gpu.maxTextureSize >= 16384 ? 2 : gpu.maxTextureSize >= 8192 ? 1 : 0;
    score += gpu.maxUniformVectors >= 512 ? 2 : gpu.maxUniformVectors >= 256 ? 1 : 0;
    ...
    if (score >= 6) {
        return { mode: 'shader-medium', score, reason: 'gpu-benchmark-medium' };
    }
...
export function modeSettings(mode) {
    switch (mode) {
        case 'shader-high':
            return { dprCap: 1.65, fpsCap: 60, strength: 1.0, maxSurfaces: 24 };
        case 'shader-medium':
            return { dprCap: 1.2, fpsCap: 45, strength: 0.76, maxSurfaces: 18 };
        case 'shader-low':
            return { dprCap: 0.9, fpsCap: 30, strength: 0.56, maxSurfaces: 12 };
        default:
            return { dprCap: 0.75, fpsCap: 24, strength: 0.0, maxSurfaces: 0 };
    }
}
